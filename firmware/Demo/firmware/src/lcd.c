...
void SERCOM2Callback(uintptr_t context) {
...
void LCDInitPort(void) {
    SERCOM2_I2C_CallbackRegister(SERCOM2Callback, (uintptr_t)NULL);
    uint8_t data = BACKLIGHT_ON;
    transferStatus = 0;
    SERCOM2_I2C_Write(LCD_I2C_ADDRESS, &data, 1);
...
void LCDWrite8(char c, char rs) {
    ...
    transferStatus = 0;
    SERCOM2_I2C_Write(LCD_I2C_ADDRESS, data, 2);
    while (!transferStatus);
}
...
void LCDWriteByte(char c, char rs) {
    ...
    data[4] = BACKLIGHT_ON;
    transferStatus = 0;
    SERCOM2_I2C_Write(LCD_I2C_ADDRESS, data, 5);
    while (!transferStatus);
}
...
char LCDReadNibble(char rs) {
    ...
    while (!transferStatus);
    transferStatus = 0;
    SERCOM2_I2C_Read(LCD_I2C_ADDRESS, &b, 1);
    while (!transferStatus);
    transferStatus = 0;
    SERCOM2_I2C_Write(LCD_I2C_ADDRESS, data, 1);
    while (!transferStatus);
